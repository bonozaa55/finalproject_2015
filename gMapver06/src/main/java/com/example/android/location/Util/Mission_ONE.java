...
public class Mission_ONE {
    ...
    public void startMission() {
        MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "ยินดีต้อนรับสู่โลกใหม่ สหายข้า! เมื่อเจ้าได้เข้าสู่โลกแห่งนี้แล้วจงเดินทางไปพบกับท่านผู้นำทั้ง 13 " +
    ...
    public void resetStateToMission() {
        String filePath = AssetsManager.getAssetPath(context,
                "TutorialDynamicModels/Assets/MarkerConfig_ONE.xml");
    ...
    public void checkMissionState() {
        ...
        switch (Mission_STATE) {
            case STATE_GET_MISSION:
                MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "\tช่วยเราด้วย ตอนนี้มีปีศาจไส้อั่วเมาแล้วอาละวาด " +
            ...
            case STATE_LOSE_TO_BOSS:
                if (Player.getHp() <= 0) {
                    MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "\tสหายยข้า! ดูเหมือนเจ้าจะเจ็บหนักนะเจ้ารีบไปรักษาตัวเถอะ ที่บ่อน้ำรูปช้างของเรา" +
                    ...
                    if (mCraftMission.getMissionStatus() != 2) {
                        MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "\tสหายยข้า! ไม่ต้องพูดอะไรข้าเข้าใจแล้วจงรับสิ่งนี้ไป ใบรายการสำหรับสร้าง" +
            ...
            case STATE_GET_CRAFT_MISSION:
                if(mCraftMission.getMissionStatus()==3){
                    MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "ว้าวว เจ้าได้สิ่งของมาครบแล้วข้าจะสร้างหนังสติ๊กให้เจ้าเอง มาๆ ส่งของทั้งหมดมา" +
                            "ฮ่าๆๆ ข้าบอกว่าทั้งหมดไงง");
                    Player.setAtkDmg(50);
                    GlobalResource.setMISSION_STATE(STATE_GET_NEW_ITEM);
                    MainActivity.makeToast("Your Atk BOOSTED!!!");
                }else {
                    MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "ไปหาสิ่งของมาให้ครบก่อน ถ้าถึงจะสร้างให้เจ้า ว่ะฮ่าๆๆๆ");
                }
                break;
            case STATE_GET_NEW_ITEM:
                MainActivity.showSimpleDialog("ผู้เฒ่ากานต์", "ข้าได้ช่วยเหลือเจ้าในทุกๆทางแล้วว ที่เหลือก็ขึ้นอยู่กับฝีมือของเจ้าแล้วละสหายข้าา " +
                        "ขอให้เจ้าจงโชคดี ว่ะฮ่าๆๆๆๆ");
                break;
            case STATE_WIN_BOSS:
                MainActivity.showSimpleDialog("ผู้เฒ่ากานต์","ยอดเยี่ยมม ยอดเยี่ยมจริงๆ แบบนี้สิไม่ผิดหวังที่ข้าฝากความหวังไว้กับเจ้า" +
                        "เอ้าา รับไปรางวัลสำหรับเจ้าาา!");
                MainActivity.makeToastItem(ItemsID.GOLD,5000);
                GlobalResource.setMISSION_STATE(STATE_COMPLETE);
                break;
            case STATE_COMPLETE:
                MainActivity.showSimpleDialog("ผู้เฒ่ากานต์","นี้ข้าหมดตัวแล้วเนี้ย เจ้าจะมาเอาอะไรกับข้าอีกอยากได้เงินก็ไปหากับพวก" +
                        "มอนเตอร์ริมถนนนู้นน");
                break;
        }
    }
